...
class Backup {
	...
	public function restore_attributes_backup_notice() {
		...
		if ( $backup && ! get_option( 'softim_variation_swatches_ignore_restore' ) ) {
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					<?php printf( esc_html__( 'Found a backup of your attributes type. This backup was generated at %s.', 'softim-variation-swatches' ), $backup_date ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'softim_variation_swatches_restore_backup', '_wpnonce' => wp_create_nonce( 'softim_variation_swatches_restore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Restore attributes', 'softim-variation-swatches' ) ?></a> |
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'softim_variation_swatches_ignore_backup', '_wpnonce' => wp_create_nonce( 'softim_variation_swatches_ignore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Ignore', 'softim-variation-swatches' ) ?></a>
				</p>
			</div>
			<?php
		}
		...
		if ( ! $backup && isset( $_GET['message'] ) && 'softim_variation_swatches_restored_backup' == $_GET['message'] ) {
			...
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e( 'All attribute types have been restored.', 'softim-variation-swatches' ) ?></p>
			</div>
			<?php
		}
	}
	...
	public function add_import_export_columns( $columns ) {
		$columns['softim_variaiton_swatches'] = esc_html__( 'Variation Swatches', 'softim-variation-swatches' );
		$columns['softim_attributes_type']    = esc_html__( 'Attributes Swatches', 'softim-variation-swatches' );
	...
	public function add_column_to_mapping_screen( $columns ) {
		$columns[ esc_html__( 'Variation Swatches', 'softim-variation-swatches' ) ]  = 'softim_variaiton_swatches';
		$columns[ esc_html__( 'Attributes Swatches', 'softim-variation-swatches' ) ] = 'softim_attributes_type';
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				...
				throw new \Exception( sprintf( esc_html__( 'Not able to attach "%s".', 'softim-variation-swatches' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			/* translators: %s: image URL */
			throw new \Exception( sprintf( esc_html__( 'Unable to use image "%s".', 'softim-variation-swatches' ), $url ), 400 );
		}
