...
class Settings {
	...
	public function get_shape_options() {
		$options = apply_filters( 'softim_variation_swatches_shape_options', [
			'round'   => esc_html__( 'Round', 'softim-variation-swatches' ),
			'rounded' => esc_html__( 'Rounded', 'softim-variation-swatches' ),
			'square'  => esc_html__( 'Square', 'softim-variation-swatches' ),
		] );
		...
		if ( current_theme_supports( 'woocommerce' ) ) {
			...
			if ( $theme_style ) {
				$options = array_merge( [ 'default' => esc_html__( 'Theme Default', 'softim-variation-swatches' ) ], $options );
			}
		}
