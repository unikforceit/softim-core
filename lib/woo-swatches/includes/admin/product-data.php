...
class Product_Data {
	...
	public function product_option_terms( $taxonomy, $index, $attribute ) {
		...
		<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'softim-variation-swatches' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $index ); ?>][]">
			<?php
			$args      = [
		...
		<button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'softim-variation-swatches' ); ?></button>
		<button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'softim-variation-swatches' ); ?></button>
		<button class="button fr plus add_new_attribute_with_swatches" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>"><?php esc_html_e( 'Add new', 'softim-variation-swatches' ); ?></button>
	...
	public function swatches_tab( $tabs ) {
		$tabs['softim_variation_swatches'] = [
			'label'    => esc_html__( 'Swatches', 'softim-variation-swatches' ),
			...
			<div id="softim_variation_swatches_data_inner" class="softim-variation-swatches-product-data  wc-metaboxes">
				...
				foreach ( $attributes as $attribute ) {
					...
					<div data-taxonomy="<?php echo esc_attr( $attribute->get_taxonomy() ); ?>" class="wc-metabox closed" rel="<?php echo esc_attr( $attribute->get_position() ); ?>">
						<h3>
							<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'softim-variation-swatches' ); ?>"></div>
						...
						<div class="wc-metabox-content hidden">
							<div class="options_group">
								...
								woocommerce_wp_select( [
									...
									'label'         => esc_html__( 'Type', 'softim-variation-swatches' ),
									'description'   => sprintf( esc_html__( 'The default type is: %s', 'softim-variation-swatches' ), $types[ $attribute_type ] ),
									'value'         => $attribute_swatches['type'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'softim-variation-swatches' ) ], $types ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								...
								woocommerce_wp_select( [
									...
									'label'         => esc_html__( 'Shape', 'softim-variation-swatches' ),
									'description'   => sprintf( esc_html__( 'The default shape is: %s', 'softim-variation-swatches' ), $shapes[ $default_settings['shape'] ] ),
									'value'         => $attribute_swatches['shape'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'softim-variation-swatches' ) ], $shapes ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								...
								woocommerce_wp_select( [
									...
									'label'         => esc_html__( 'Size', 'softim-variation-swatches' ),
									'value'         => $attribute_swatches['size'],
									'options'       => [
										''       => esc_html__( 'Default', 'softim-variation-swatches' ),
										'custom' => esc_html__( 'Custom', 'softim-variation-swatches' ),
									],
								] );
								?>
								<p class="form-field form-field--custom-size dimensions_field <?php echo 'custom' != $attribute_swatches['size'] ? 'hidden' : '' ?>">
									<span class="wrap">
										<input type="text" name="softim_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][width]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['width'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Width', 'softim-variation-swatches' ) ?>">
										<input type="text" name="softim_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][height]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['height'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Height', 'softim-variation-swatches' ) ?>">
									</span>
								</p>
							</div>
			...
			switch ( $args['type'] ) {
				...
				case 'image':
					...
					printf(
						...
						esc_html( $label ),
						esc_html__( 'Edit', 'softim-variation-swatches' ),
						empty( $values['image'] ) ? 'hidden' : '',
						esc_html__( 'Delete Image', 'softim-variation-swatches' ),
						esc_html__( 'Remove', 'softim-variation-swatches' ),
	...
	public function ajax_add_new_attribute_term() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'softim_variation_swatches_add_term' ) ) {
			wp_send_json_error( esc_html__( 'Wrong request', 'softim-variation-swatches' ) );
		}

		if ( empty( $_POST['attribute_name'] ) || empty( $_POST['attribute_taxonomy'] ) ) {
			wp_send_json_error( esc_html__( 'Not enough data', 'softim-variation-swatches' ) );
		}
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'Taxonomy is not exists', 'softim-variation-swatches' ) );
		}

		if ( term_exists( $name, $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'This term is already exists', 'softim-variation-swatches' ) );
		}
		...
		if ( ! is_wp_error( $term ) ) {
			wp_send_json_success( [
				'message' => esc_html__( 'Added successfully', 'softim-variation-swatches' ),
			...
			<div class="softim-variation-swatches-modal wp-core-ui" tabindex="0" role="dialog">
				<button type="button" class="media-modal-close">
					<span class="media-modal-icon">
						<span class="screen-reader-text"><?php esc_html_e( 'Close dialog', 'softim-variation-swatches' ) ?></span>
					</span>
				</button>
				<div class="softim-variation-swatches-modal__header"><h2><?php esc_html_e( 'Add New Term', 'softim-variation-swatches' ) ?></h2></div>
				<div class="softim-variation-swatches-modal__content">
					<p class="form-field">
						<label>
							<?php esc_html_e( 'Name', 'softim-variation-swatches' ) ?><br>
					...
					<p class="form-field form-field__swatches">
						<span class="softim-variation-swatches__field-color">
							<span class="label"><?php esc_html_e( 'Color', 'softim-variation-swatches' ) ?></span><br>
							<input type="text" name="color" value="">
						</span>
						<span class="softim-variation-swatches__field-image">
							<span class="label"><?php esc_html_e( 'Image', 'softim-variation-swatches' ) ?></span><br>
							<img src="<?php echo esc_url( $default_image ) ?>" data-placeholder="<?php echo esc_url( $default_image ) ?>" width="60" height="60">
							<a href="#" class="button-add-image"><?php esc_html_e( 'Upload', 'softim-variation-swatches' ) ?></a>
							<a href="#" class="button-link button-remove-image delete hidden"><?php esc_html_e( 'Delete', 'softim-variation-swatches' ) ?></a>
							<input type="hidden" name="image" value="">
						</span>
						<span class="softim-variation-swatches__field-label">
							<span class="label"><?php esc_html_e( 'Label', 'softim-variation-swatches' ) ?></span><br>
				...
				<div class="softim-variation-swatches-modal__footer">
					<button type="button" class="button-add button button-primary"><?php esc_html_e( 'Add New', 'softim-variation-swatches' ) ?></button>
