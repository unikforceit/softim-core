...
jQuery( function( $ ) {
	...
	var SoftimVariationSwatchesMeta = {
		init: function() {
			$( '.softim-variation-swatches__field-color input' ).wpColorPicker();

			$( document.body )
				.on( 'click', '.softim-variation-swatches__field-image .button-add-image, .softim-variation-swatches__field-image img', this.uploadImage )
				.on( 'click', '.softim-variation-swatches__field-image .button-remove-image', this.removeImage );

			$( '#softim_variation_swatches_data' )
				.on( 'change', '.softim-variaton-swatches__type-field select', this.toggleSwatchesMetabox )
				.on( 'change', '.softim-variaton-swatches__size-field select', this.toggleCustomSizeFields );
			...
			$( '#softim-variation-swatches-new-term-dialog' )
		...
		uploadImage: function( event ) {
			...
			mediaFrame.on( 'select', function () {
				...
				$button.closest( '.softim-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( attachment.id );
				$button.siblings( '.button-remove-image' ).removeClass( 'hidden' );

				var attachment_image = attachment.sizes && attachment.sizes.thumbnail ? attachment.sizes.thumbnail.url : attachment.url;
				$button.closest( '.softim-variation-swatches__field-image' ).find( 'img' ).attr( 'src', attachment_image );
			} );
		...
		removeImage: function( event ) {
			...
			$button.closest( '.softim-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( '' );
			$button.closest( '.softim-variation-swatches__field-image' ).find( 'img' ).attr( 'src', function() {
		...
		reloadSwatchesPanel: function() {
			...
			$( '#softim_variation_swatches_data' ).load( this_page + ' #softim_variation_swatches_data_inner', function() {
				setTimeout( function() {
					$( '.softim-variation-swatches__field-color input' ).wpColorPicker();
				} );
			} );
		...
		closeDialog: function( event ) {
			...
			$( '#softim-variation-swatches-new-term-dialog' ).hide();
		},
		...
		openNewAttributeDialog: function( event ) {
			...
			var $button = $( this ),
				$dialog = $( '#softim-variation-swatches-new-term-dialog' ),
				data = {
			...
			$( '.form-field__swatches', $dialog ).children().hide().filter( '.softim-variation-swatches__field-' + data.type ).show();
		...
		ajaxAddTerm: function( event ) {
			...
			var $button = $( this ),
				...
				$message = $( '.softim-variation-swatches-modal__message', $dialog ),
			...
			wp.ajax.send( 'softim_variation_swatches_add_term', {
				data   : data,
				error  : function ( res ) {
	...
	SoftimVariationSwatchesMeta.init();
} );