;( function( $, document ) {
	var VariationSwatches = function( $form ) {
		...
		// Initial states.
		$form.off( '.softim-variation-swatches' );

		// Add a new CSS to the form.
		if ( $form.find( '.softim-variation-swatches__wrapper' ).length ) {
			$form.addClass( 'swatches-support' );
		}

		// Events.
		$form.on( 'click.softim-variation-swatches', '.softim-variation-swatches__item', { variationSwatches: self }, self.onSelect );
		$form.on( 'keydown.softim-variation-swatches', '.softim-variation-swatches__item', { variationSwatches: self }, self.onKeyPress );
		$form.on( 'click.softim-variation-swatches', '.reset_variations', { variationSwatches: self }, self.onReset );
		$form.on( 'woocommerce_update_variation_values', { variationSwatches: self }, self.onUpdateAttributes );

		if ( softim_variation_swatches_params.show_selected_label ) {
		...
		$( document.body ).trigger( 'softim_variation_swatches', self );
	};
	...
	VariationSwatches.prototype.onSelect = function( event ) {
		...
		var $el = $( this ),
			$select = $el.closest( '.softim-variation-swatches' ).find( 'select' ),
			value = $el.data( 'value' );
	...
	VariationSwatches.prototype.onKeyPress = function( event ) {
		if ( event.keyCode && 32 === event.keyCode || event.key && ' ' === event.key || event.keyCode && 13 === event.keyCode || event.key && 'enter' === event.key.toLowerCase() ) {
			...
			$( this ).trigger( 'click.softim-variation-swatches' );
		}
	}
	...
	VariationSwatches.prototype.onReset = function( event ) {
		event.preventDefault();
		event.data.variationSwatches.$form.find( '.softim-variation-swatches__item.selected' ).removeClass( 'selected' );
		event.data.variationSwatches.$form.find( '.softim-variation-swatches__item.disabled' ).removeClass( 'disabled' ).data( 'disabled', false );
	};
	...
	VariationSwatches.prototype.onUpdateAttributes = function( event ) {
		setTimeout( function() {
			event.data.variationSwatches.$form.find( '.softim-variation-swatches' ).each( function( index, row ) {
				var $swatches = $( row ),
				...
				$swatches.find( '.softim-variation-swatches__item' ).each( function( index, item ) {
					var $item = $( item ),
	...
	VariationSwatches.prototype.appendSelectedLabel = function( select ) {
		var $label = $( select ).closest( '.value' ).siblings( '.label' ).find( 'label' ),
			$holder = $label.find( '.softim-variation-swatches__selected-label' );

		if ( ! $holder.length ) {
			$holder = $( '<span class="softim-variation-swatches__selected-label" />' );
	...
	/**
	 * Function to call softim_variation_swatches on jquery selector.
	 */
	$.fn.softim_variation_swatches = function() {
	...
	function init_variation_swatches() {
		$( '.variations_form:not(.swatches-support)' ).each( function() {
			$( this ).softim_variation_swatches();
		} );
	...
	$( function() {
		...
		$( document.body ).trigger( 'softim_variation_swatches_initialized' );
	} );
} )( jQuery, document )
