...
class Softim_Contact_Info_Widget extends WP_Widget{

	public function __construct() {
		parent::__construct(
			'softim_contact_info',
			esc_html__('Softim: Contact Info','softim-core'),
			array('description' => esc_html__('Display contact info widget','softim-core'))
		);
	}
	...
	public function form($instance){
		...
		<?php foreach ( $contact_info as $ci ) :?>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( $ci ) ); ?>"><?php echo esc_html( ucfirst( $ci ) . " " . esc_html__( 'Info', 'softim-core' ) ); ?>
					: </label>
				<br/>
				...
				<small><?php echo esc_html__('Leave it blank if you don\'t want to show this info','softim-core')?></small>
			</p>
...
if (!function_exists('Softim_Contact_Info_Widget')){
	function Softim_Contact_Info_Widget(){
		register_widget('Softim_Contact_Info_Widget');
	}
	add_action('widgets_init','Softim_Contact_Info_Widget');
}
