...
if (!class_exists('Softim_Core_Admin_Menu')) {

    class Softim_Core_Admin_Menu
    {
       /**
        * $instance
        * @since 1.0.0
        ...
        */
        ...
        public function theme_admin_menu_page()
        {
            ...
            //add menu page
            add_menu_page(
                esc_html__('Softim Page', 'softim-core'),
                esc_html__('Softim', 'softim-core'),
                'manage_options',
                'softim_theme_options',
                array($this, 'admin_options_fallback_function'),
                SOFTIM_CORE_ADMIN_ASSETS . '/img/icon.png',
                80
            );
        }
            ...
            $tab_menus = array(
                'service' => array(
                    array(
                        ...
                        'name' => sprintf(esc_html__('%s', 'softim-core'), 'Service'),
                        'id' => 'edit-service'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=service-cat&post_type=service',
                        'name' => sprintf(esc_html__('%s Categories', 'softim-core'), 'Service'),
                        'id' => 'edit-service-cat'
                    )
                ),
                'packages' => array(
                ...
                'team' => array(
                    array(
                        ...
                        'name' => sprintf(esc_html__('%s', 'softim-core'), 'Team'),
                        'id' => 'edit-team'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=team-cat&post_type=team',
                        'name' => sprintf(esc_html__('%s Categories', 'softim-core'), 'Team'),
                        'id' => 'edit-team-cat'
                    )
                )
            );
        ...
        public function admin_menu_activation()
        {
            ...
            <script type="text/javascript">
                (function ($) {
                    ...
                    mainwrap = $('#toplevel_page_softim_theme_options');
                    ...
                    if (check) {
                        $('.wp-submenu.wp-submenu-wrap')
                            .find('a[href*="admin.php?page=toplevel_page_softim_theme_options"]')
                            .removeClass('current')
                            .parent('li')
                            .removeClass('current');
                    }
    ...
    if (class_exists('Softim_Core_Admin_Menu')) {
        Softim_Core_Admin_Menu::getInstance();
    }
