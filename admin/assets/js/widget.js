;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         harnold Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('harnold_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.harnold_flogo_uploader');

        $('body').on('click','.harnold_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.harnold-logo-preview');
                ...
                $(el).siblings().children('.harnold_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.harnold_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.harnold_logo_id').val());
                var atts = $(el).siblings().children('.harnold_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.harnold_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.harnold-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.harnold_flogo_uploader').val('Change Image');
